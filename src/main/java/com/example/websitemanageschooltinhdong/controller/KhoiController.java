...
public class KhoiController {
    ...
    @GetMapping("/monhoc/{idmon}")
    public ResponseEntity<Integer> getIdKhoiByMonHoc(@PathVariable("idmon") String id) {
        Integer idkhoi = khoiRepository.findIdKhoiByMonHocId(id);
        if (idkhoi == null) {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
        return new ResponseEntity<>(idkhoi, HttpStatus.OK);
    }
}
